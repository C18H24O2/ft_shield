...
StringRef readAnnotate(Function *f, StringRef attr) {
  ...
  // Get the array
  auto *ca = cast<ConstantArray>(glob->getOperand(0));
  if (!ca)
    return "";

  for (Value *Op : ca->operands()) {
    // Get the struct
    auto *structAn = cast<ConstantStruct>(Op);

    auto *AnnotatedVar = structAn->getOperand(0)->stripPointerCasts();
    if (AnnotatedVar != f)
      continue;

    auto *annoteStr =
        cast<GlobalVariable>(structAn->getOperand(1)->stripPointerCasts());
    if (!annoteStr)
      continue;
    ...
    while (true) {
      ...
      if (attr_pos == std::string::npos)
        break;
      if (attr_pos != 0 && !std::isspace(attribute[attr_pos - 1]))
        continue;
      ...
      if (assign != std::string::npos) {
        ...
        while (std::isspace(attribute[value]))
          ++value;
        auto end = value + 1;
        while (end < attribute.size() && !std::isspace(attribute[end]) &&
               attribute[end] != '\0')
          ++end;
